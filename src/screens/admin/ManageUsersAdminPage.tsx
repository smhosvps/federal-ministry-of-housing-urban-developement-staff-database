...
export default function ManageUsersAdminPage() {
  ...
  const { filteredUsers, totalPages, totalFilteredUsers } = useMemo(() => {
    ...
    if (filters.status !== "all") {
      filtered = filtered.filter((user:any) => user.status === filters.status);
    }

    // Approval filter
    if (filters.is_approved !== "all") {
      filtered = filtered.filter(
        (user:any) => user.is_approved === filters.is_approved
      );
    }
  ...
  const exportToCSV = () => {
    ...
    const csvData = allUsers.map((user:any, index) => [
      index + 1,
  ...
  const pendingApprovalCount = useMemo(
    () => allUsers.filter((u:any) => u.is_approved === "pending").length,
    [allUsers]
  );

  const adminUsersCount = useMemo(
    () =>
      allUsers.filter((u:any) => ["admin", "Super Admin"].includes(u.role)).length,
    [allUsers]
  );

  const deletedUsersCount = useMemo(
    () => allUsers.filter((u:any) => u.status === "deleted").length,
    [allUsers]
  );

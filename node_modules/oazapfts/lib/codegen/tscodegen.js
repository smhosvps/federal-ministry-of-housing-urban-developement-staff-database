...
function createIntefaceAliasDeclaration({ modifiers, name, typeParameters, type, inheritedNodeNames, }) {
    const heritageClauses = inheritedNodeNames
        ? [
            typescript_1.factory.createHeritageClause(typescript_1.default.SyntaxKind.ExtendsKeyword, inheritedNodeNames.map((name) => {
                const extendedInterfaceName = typeof name === "string" ? name : name.escapedText.toString();
                return typescript_1.factory.createExpressionWithTypeArguments(typescript_1.factory.createIdentifier((0, generate_1.toIdentifier)(extendedInterfaceName, true)), undefined);
            })),
        ]
        : [];
    return typescript_1.factory.createInterfaceDeclaration(modifiers, name, typeParameters, heritageClauses, type.members);
}
...
function createClassDeclaration({ modifiers, name, typeParameters, heritageClauses, members, }) {
    return typescript_1.factory.createClassDeclaration(modifiers, name, typeParameters, heritageClauses, members);
}
exports.createClassDeclaration = createClassDeclaration;
function createConstructor({ modifiers, parameters, body, }) {
    return typescript_1.factory.createConstructorDeclaration(modifiers, parameters, body);
}
